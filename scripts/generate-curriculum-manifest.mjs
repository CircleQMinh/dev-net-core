...
const seoOutputRelativePath = toPosix(
  path.relative(projectRoot, seoOutputPath)
);
...
const usedCanonicalPaths = new Map();
const entries = markdownFiles.map(({ absolutePath, relativePath }) => {
  ...
  const customCanonicalPath = frontmatter.canonicalPath?.trim();
  const customSeoDescription = frontmatter.seoDescription?.trim();
  const customSeoTitle = frontmatter.seoTitle?.trim();
  const canonicalPath = customCanonicalPath || `/content/${id}/`;

  validateCanonicalPath(canonicalPath, relativePath);
  registerCanonicalPath(canonicalPath, relativePath, usedCanonicalPaths);
  ...
  return {
    canonicalPath,
    canonicalPathSource: customCanonicalPath ? "frontmatter" : "generated",
    category,
    ...
    seoDescription:
      ...
      `Learn ${subtopicTitle} for technical interviews, including core concepts, practical examples, common mistakes, and interview questions.`,
    seoDescriptionSource: customSeoDescription ? "frontmatter" : "fallback",
    seoTitle:
      customSeoTitle ||
      `${subtopicTitle} Interview Questions | DEV_NET_CORE`,
    seoTitleSource: customSeoTitle ? "frontmatter" : "fallback",
...
const missingSeoTitleCount = entries.filter(
  (entry) => entry.seoTitleSource === "fallback"
).length;
const missingSeoDescriptionCount = entries.filter(
  (entry) => entry.seoDescriptionSource === "fallback"
).length;

if (missingSeoTitleCount > 0 || missingSeoDescriptionCount > 0) {
  console.warn(
    "SEO metadata transition fallbacks are active: " +
      `${missingSeoTitleCount} title(s), ` +
      `${missingSeoDescriptionCount} description(s).`
  );
}

const typescriptOutput = `// This file is generated by scripts/generate-curriculum-manifest.mjs.
...
const seoOutput = `${JSON.stringify(
  {
    ...
    entries: entries.map((entry) => ({
      ...
      canonicalPathSource: entry.canonicalPathSource,
      category: entry.category,
      contentPath: entry.contentPath.replace(/^\/+/, ""),
      id: entry.id,
      seoDescription: entry.seoDescription,
      seoDescriptionSource: entry.seoDescriptionSource,
      seoTitle: entry.seoTitle,
      seoTitleSource: entry.seoTitleSource,
...
fs.writeFileSync(outputPath, typescriptOutput);
...
console.log(
  `Generated ${entries.length} curriculum SEO entries in ${seoOutputRelativePath}`
);
...
function validateCanonicalPath(canonicalPath, contentPath) {
  if (!canonicalPath.startsWith("/")) {
    throw new Error(
      `Canonical path "${canonicalPath}" in ${contentPath} must start with "/".`
    );
  }

  if (!canonicalPath.endsWith("/")) {
    throw new Error(
      `Canonical path "${canonicalPath}" in ${contentPath} must use a trailing slash.`
    );
  }
  ...
  if (
    ...
    canonicalPath.includes("\\") ||
    canonicalPath.includes("//") ||
    !/^\/content\/[a-z0-9][a-z0-9-]*\/$/.test(canonicalPath)
  ) {
    throw new Error(
      `Canonical path "${canonicalPath}" in ${contentPath} must match ` +
        '"/content/topic-slug/".'
    );
  }
}

function registerCanonicalPath(canonicalPath, contentPath, usedCanonicalPaths) {
  const existingContentPath = usedCanonicalPaths.get(canonicalPath);

  if (existingContentPath) {
    throw new Error(
      `Duplicate canonical path "${canonicalPath}" found in ` +
        `${existingContentPath} and ${contentPath}.`
    );
  }

  usedCanonicalPaths.set(canonicalPath, contentPath);
}
